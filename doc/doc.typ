...
XLang-Rust 由于先天缺乏具名类型系统，因此在实现一些复杂的类型时会显得非常麻烦。通常的工厂模式会导致大量重复对象的定义和代码的冗余，并且在类型检查时产生显著的设计困难（通过alias和get\_type方法来实现类型检查）。因此XLang-Rust引入了针对键值对的特殊成员访问运算。暂时称为Mixin。

Mixin允许在一个键值对通过 `.` 运算符按照一定的顺序访问键和值的成员，顺序如下：

+ 首先尝试在值中查找成员
+ 如果值中没有，则尝试在键中查找成员
+ 如果值中找到的成员是一个函数，则浅拷贝一个新的lambda对象并将lambda的self引用绑定为键值对对象, 否则返回原始对象
+ 对键中的所有成员都返回原始对象
...
obj := A : mixin;
io.print(obj.get_a()); // 输出 1
io.print(obj.get_b()); // 输出 2
obj.d(); // 输出 1
io.print(obj.c); // 输出 3
```
