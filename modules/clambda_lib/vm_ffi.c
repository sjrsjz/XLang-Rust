...
void* clambda_entry(void* lookup_fn) {
    ...
    printf("C Lambda initialized\n");
    return NULL;
}

// CLambda 销毁函数 - 清理资源
void clambda_destroy(void) {
    printf("C Lambda destroyed\n");
    rust_lookup = NULL;
}
...
// 示例CLambda函数实现
FFIGCRef clambda___main__(FFIGCRef gc_ref, void* gc_system) {
    // 打印输入类型信息
    printf("C Lambda example function called\n");
    printf("输入类型: ");
    print_value_info(gc_ref);
    ...
    long long size = get_len(gc_ref);
    printf("元组大小: %lld\n", size);
    
    for (long long i = 0; i < size; i++) {
        FFIGCRef value = vm_tuple_get(gc_ref, i, gc_system);
        if (!value.data) {
            printf("Error: 元组元素[%lld] 获取失败\n", i);
            continue;
        }
    ...
    printf("C Lambda example function completed\n");
...
FFIGCRef clambda_add(FFIGCRef gc_ref, void* gc_system) {
    if(!is_vm_tuple(gc_ref)) {
        printf("Error: Expected a tuple\n");
        return FFIGCRef_null();
    }
    ...
    if (size != 2) {
        printf("Error: Expected a tuple of size 2\n");
        return FFIGCRef_null();
    }
    
    FFIGCRef value1 = vm_tuple_get(gc_ref, 0, gc_system);
    FFIGCRef value2 = vm_tuple_get(gc_ref, 1, gc_system);
    
    if (!value1.data || !value2.data) {
        printf("Error: Failed to get tuple elements\n");
        return FFIGCRef_null();
    }
    
    if (!is_vm_int(value1) || !is_vm_int(value2)) {
        printf("Error: Expected both elements to be integers\n");
        return FFIGCRef_null();
    }
    
    int64_t sum = get_vm_int_value(value1) + get_vm_int_value(value2);
    
    // 创建新的整数对象并返回
    FFIGCRef result = new_vm_int64(sum, gc_system);
    return result;
}