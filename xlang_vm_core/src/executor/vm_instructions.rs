pub mod vm_instructions {
    ...
    pub fn load_lambda(
        ...
        if let OpcodeArgument::String(sign_idx) = opcode.operand1 {
            if let OpcodeArgument::Int64(code_position) = opcode.operand2 {
                let mut instruction = vm.get_object_and_check(0)?; // Changed
                let mut idx = 1;
                let mut capture = None;
                if let OpcodeArgument::Int32(should_capture) = opcode.operand3 {
                    if should_capture != 0 {
                        capture = Some(vm.get_object_and_check(1)?); // Changed
                        idx = 2;
                    }
                }
                let mut default_args_tuple = vm.get_object_and_check(idx)?; // Changed
    ...
    pub fn bind_self(
        ...
        let mut obj = vm.get_object_and_check(0)?; // Changed
    ...
    pub fn build_keyval(
        ...
        let mut value = vm.get_object_and_check(0)?; // Changed
        let mut key = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn binary_add(
        ...
        let mut right = vm.get_object_and_check(0)?; // Changed
        let mut left = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn unary_minus(
        ...
        let mut ref_obj = vm.get_object_and_check(0)?; // Changed
    ...
    pub fn set_var(
        ...
        let mut value = vm.get_object_and_check(0)?; // Changed
        let mut reference = vm.get_object_and_check(1)?; // Changed
        let result = try_assign_as_vmobject(&mut reference, &mut value).map_err(VMError::VMVariableError)?;
    ...
    pub fn raise(
        ...
        let obj = vm.get_object_and_check(0)?; // Changed
    ...
    pub fn get_attr(
        ...
        let mut attr = vm.get_object_and_check(0)?; // Changed
        let mut obj = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn index_of(
        ...
        let mut index = vm.get_object_and_check(0)?; // Changed
        let mut obj = vm.get_object_and_check(1)?; // Changed
        let result =
    ...
    pub fn call_lambda(
        ...
        let mut arg_tuple = vm.get_object_and_check(0)?; // Changed
        let mut lambda = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn is_in(
        ...
        // Pop objects from stack after successful operation (implicit pops were missing before)
        vm.pop_object()?;
        vm.pop_object()?;
        vm.push_vmobject(gc_system.new_object(VMBoolean::new(result)))?;

        // Drop references at the end (were missing before)
        container.drop_ref();
        obj.drop_ref();
        Ok(None)
    }
    ...
    pub fn build_range(
        ...
        let mut end = vm.get_object_and_check(0)?; // Changed
        let mut start = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn type_of(
        ...
        let type_str = if ref_obj.isinstance::<VMInt>() {
            ...
            "bytes"
        } else if ref_obj.isinstance::<VMSet>() { // Added VMSet
            "set"
        } else {
            ""
        };
    ...
    pub fn import(
        ...
        let mut path_arg = vm.get_object_and_check(0)?; // Changed
    ...
    pub fn alias_of(
        ...
        // Drop references at the end
        for mut alias_obj in tuple { // Changed loop variable name
            alias_obj.drop_ref();
        }
        obj.drop_ref();
        Ok(None)
    ...
    pub fn build_set(
        ...
        let mut filter = vm.get_object_and_check(0)?; // Changed
        let mut collection = vm.get_object_and_check(1)?; // Changed
    ...
    pub fn push_value_into_tuple(
        ...
        let mut tuple = vm.get_object_and_check((offset + 1) as usize)?; // Changed: Get tuple using offset + 1
