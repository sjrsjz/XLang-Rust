...
pub fn print(tuple: &mut GCRef, gc_system: &mut GCSystem) -> Result<GCRef, VMVariableError> {
...
pub fn input(tuple: &mut GCRef, gc_system: &mut GCSystem) -> Result<GCRef, VMVariableError> {
    check_if_tuple(tuple)?;
    let tuple_obj = tuple.as_type::<VMTuple>();
    if tuple_obj.values.len() > 1 {
         return Err(VMVariableError::TypeError(
            tuple.clone_ref(),
            format!("input expected 0 or 1 arguments, got {}", tuple.as_const_type::<VMTuple>().values.len()),
        ));
    }
